...
from utiles import alignMeasurements
...
def main(argv):
    ...
    figFilename = "results/phaseDeviationssWCoupledINapIKI0%.02fEpsilon%.02fCouplingStart%.02f.eps"%(i0, epsilon, couplingStartTime)

    res = np.load(integrationFilename)
    times = res["times"]
    voltagesNeuron1 = res["ys"][0,:]
    voltagesNeuron2 = res["ys"][2,:]
    timesUncoupledNeuron1 = res["timesUncoupledModel1"]
    voltagesUncoupledNeuron1 = res["ysUncoupledModel1"][0,:]
    ...
    resUncoupledNeuron1 = getPhasesFromVoltages(times=timesUncoupledNeuron1, voltages=voltagesUncoupledNeuron1)
    phasesUncoupledNeuron1 = resUncoupledNeuron1["phases"]*2*np.pi
    timePhasesUncoupledNeuron1 = resUncoupledNeuron1["times"]
    T = np.mean(resUncoupledNeuron1["ts"])
