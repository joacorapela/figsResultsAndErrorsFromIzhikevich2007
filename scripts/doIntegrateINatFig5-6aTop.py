...
def main(argv):
    ...
    ltINatModel = INatModel.getLowThresholdInstance(i=i)
    integrator = ode(ltINatModel.deriv).set_integrator('vode', max_step=dt)
    ...
    plt.savefig('figures/phaseSpaceIntegrationINatFig5-6aTop.eps')
    plt.figure()
    plt.plot(times, ys[0, :])
    plt.grid()
    plt.xlabel('Time (secs)')
    plt.ylabel('Voltage (mv)')
    plt.savefig('figures/voltageTraceIntegrationINatFig5-6aTop.eps')
    plt.show()
    pdb.set_trace()
