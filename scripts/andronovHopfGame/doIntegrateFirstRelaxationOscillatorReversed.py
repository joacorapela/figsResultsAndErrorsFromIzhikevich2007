...
def main(argv):
    ...
    lcStartTime = 20.0
    traceCol = 'grey'
    resultsFilename = 'results/integrationFirstRelaxationOscillatorSubcriticalBeforeLossOfStabilityUnstableLC.npz'
    figFilename = 'figures/phaseSpaceFirstRelaxationOscillatorSubcriticalBeforeLossOfStabilityUnstableLC.eps'
    ...
    reversedDeriv = lambda t, y: -model.deriv(t, y)
    integrator = ode(reversedDeriv).set_integrator('vode')
    ...
    lcTimeIndices = np.where(times>=lcStartTime)
    times = times[lcTimeIndices]
    ys = ys[:, lcTimeIndices]
