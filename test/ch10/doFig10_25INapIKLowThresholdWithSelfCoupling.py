...
def unwrapPhaseDifs(phaseDifs, T):
    unwrappedPhaseDifs= phaseDifs
    wrappedPhaseIndices = np.where(phaseDifs<0)
    unwrappedPhaseDifs[wrappedPhaseIndices] = phaseDifs[wrappedPhaseIndices]+T
    return unwrappedPhaseDifs
...
def main(argv):
    ...
    linestyleCoupled = "-"
    linestyleUncoupled = ":"
    integrationFilename = "results/integrationWCoupledINapIKLowThresholdWithSelfCouplingStrength%.02fI0%.02fEpsilon%.06fCouplingStartTime%.02fV00%.02fN00%.02fV01%.02fN01%.02f.npz"%(selfCouplingStrength, i0, epsilon, couplingStartTime, v00, n00, v01, n01)
    figFilenamePattern = "figures/fig10_25INapIKLowThresholdWithSelfCouplingStrength%.02fI0%.02fEpsilon%.06fCouplingStart%.02fV00%.02fN00%.02fV01%.02fN01%.02fxlim%.02f_%.02f.%s"
    ...
    resPhasesUncoupledNeuron0 = \
     getPhasesFromVoltages(times=timesUncoupledNeuron0, 
                            voltages=voltagesUncoupledNeuron0)
    phasesUncoupledNeuron0 = resPhasesUncoupledNeuron0["phases"]
    timePhasesUncoupledNeuron0 = resPhasesUncoupledNeuron0["times"]
    ...
    phaseDifs = unwrapPhaseDifs(phaseDifs=phaseDifs, T=T)
