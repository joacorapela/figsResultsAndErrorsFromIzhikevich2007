...
def malkinsHFunc(qsi, qsiPhases, xsi, xsiPhases, xsj, xsjPhases, coupling, phaseDiff, T):
    def integrand(theta):
        qsiPhaseIndex = np.argmin(np.abs(theta-qsiPhases))
        xsiPhaseIndex = np.argmin(np.abs(theta-xsiPhases))
        xsjPhaseIndex = np.argmin(np.abs((theta+phaseDiff)%T-xsjPhases))
        couplingVector = coupling(xsi[:, xsiPhaseIndex], 
                                   xsj[:, xsjPhaseIndex])
        answer = qsi[:,qsiPhaseIndex].dot(couplingVector)
        # if phaseDiff>3.0:
            # pdb.set_trace()
        return answer
